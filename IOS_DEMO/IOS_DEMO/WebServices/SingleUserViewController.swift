...
extension SingleUserViewController {
    ...
    func getSingleUserData(itemIndex:Int){
        AF.request("\(Constants.singleUserAPI)\(itemIndex)", parameters: nil, headers: nil).validate(statusCode: Constants.twoHundred ..< Constants.twoNintynine).response{ AFdata in
            do {
                print(AFdata.data)
                ...
                if let url = URL(string: receivedResponse.data.avatar) {
                    guard let data = try? Data(contentsOf: url) else {
                        print(Constants.canNotConvertJsonObject)
                        return }
